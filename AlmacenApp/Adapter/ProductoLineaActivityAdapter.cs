...
{
    class ProductoLineaActivityAdapter : BaseAdapter<ProductoLineaErp>
    {

        private List<ProductoLineaErp> listItems;
        Context myContext;
        LineaActivity lineaActivity;

        public ProductoLineaActivityAdapter(Context context, List<ProductoLineaErp> items, LineaActivity linea)
        {
            listItems = items;
            myContext = context;
            lineaActivity = linea;
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            try
            {
                imageDeletelinea.Click += (sender, args) => lineaActivity.EliminaLinea(sender, listItems[position].id_producto_linea.ToString(), listItems[position].descripcion.ToString(), listItems[position].cant_tipos.ToString());
            }
            catch (Exception ex)
            {
